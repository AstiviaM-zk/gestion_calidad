/**
 * Vue 3 DocumentManager Component
 */
const DocumentManager = {
  ...
  computed: {
    filteredDocuments() {
      return this.documents.filter(doc => {
        const matchesFilter = this.activeFilter === 'all' || doc.status === this.activeFilter;
  ...
  methods: {
    ...
    viewDoc(doc) {
      alert(`Visualizando documento: ${doc.title} (${doc.id})`);
    },
    downloadDoc(doc) {
      alert(`Descargando archivo: ${doc.title} [${doc.size}]`);
    },
    editDoc(doc) {
      alert(`Editando metadatos de: ${doc.id}`);
    },
    createDocument() {
      alert('Abrir formulario de alta de nuevo documento QMS');
    }
  }
};
