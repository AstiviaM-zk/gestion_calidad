...
const UserProfile = {
  ...
  computed: {
    avatarUrl() {
      return this.user?.picture || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(this.user?.name || 'User') + '&background=6366f1&color=fff';
    }
  },
  methods: {
    handleAvatarError(e) {
      e.target.src = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(this.user?.name || 'User') + '&background=6366f1&color=fff';
    },
    async copyToken() {
      if (this.token) {
        try {
